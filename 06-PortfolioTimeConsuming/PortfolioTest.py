...
class AsyncComputation:
    def __init__(self, closures):
        ...
        self._async_results = [self._pool.apply_async(c[0], c[1]) for c in closures]
    
    def get(self, n):
        return self._async_results[n].get()
...
class AccountSummaryWithInvestmentEarnings:
    ...
    def lines(self):
        ...
        computation = AsyncComputation([
            (lambda s: s.lines(), (summary,)),
            (lambda ie: ie.value(), (investmentEarnings,))
            ])

        lines = computation.get(0)
        lines.append("Ganancias por " + str(computation.get(1)))
        return lines
...
class AccountSummaryWithAllInvestmentInformation:
    ...
    def lines(self):
        ...
        computation = AsyncComputation([
            (lambda s: s.lines(), (summary,)),
            (lambda inet: inet.value(), (investmentNet,))
            ])

        lines = computation.get(0)
        lines.append("Inversiones por " + str(computation.get(1)))
        return lines
